...
> 多租户 SaaS 身份管理 — Next.js 15 App Router 全栈（前端 + IdP Route Handlers + Drizzle DB）。
> consumes `saas-identity-platform-shared` TypeSpec SSOT。字段照抄 BASE（ADR-0024）；结构性对齐按 shared 9/7 重构。
...
| M04.F03.I07 | 授权码签发（OLD ID） | 接口 | 仅后端 | 旧版：OAuth authorize 端点 → **M04.F03.I01** | 已废弃 |
| M04.F03.I08 | 令牌交换（OLD ID） | 接口 | 仅后端 | 旧版：OAuth token grantType=authorization_code → **M04.F03.I02** | 已废弃 |
| M04.F03.I09 | 令牌刷新（OLD ID） | 接口 | 仅后端 | 旧版：OAuth token grantType=refresh_token → **M04.F03.I03** | 已废弃 |
