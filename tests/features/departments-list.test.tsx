...
import { DepartmentsClient } from "@/app/(protected)/departments/departments-client";
import { seedDatabase } from "@/db/seed";
import { fnTest } from "../fn";

/**
 * M02.F01 多租户切换 — 部门管理页（fn-ID M02.F01.I01-I06）
 *
 * 覆盖：I01 页面根 / I02 树形查询 / I03-I04 新增 / I05 编辑 / I06 删除
 *
 * v0.3.0 重命名（原 orgs-list → departments-list，URL /api/orgs → /api/departments）。
 */
afterEach(() => {
  cleanup();
  vi.restoreAllMocks();
...
describe("M02.F01 departments list page", () => {
  fnTest(["M02.F01.I01"], "页面根挂 data-fn M02.F01.I01", () => {
    const { getByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
  ...
  fnTest(["M02.F01.I03", "M02.F01.I04"], "新增根 / 子按钮挂 data-fn", () => {
    const { getAllByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    const newRootBtns = getAllByTestId("new-department-btn");
    expect(newRootBtns[0]!.getAttribute("data-fn")).toMatch(/M02\.F01\.I0[34]/);
  });

  fnTest(["M02.F01.I05"], "编辑按钮挂 data-fn M02.F01.I05", () => {
    const { getAllByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    expect(getAllByTestId("edit-department-btn")[0]!.getAttribute("data-fn")).toBe("M02.F01.I05");
  });
  ...
  fnTest(["M02.F01.I06"], "删除按钮点击触发 fetch DELETE", async () => {
    ...
    const { getAllByTestId, getByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    const deleteBtns = getAllByTestId("delete-department-btn");
  ...
  fnTest(["M02.F01.I03"], "I03 新增根部门按钮挂 data-fn + 打开 Dialog", async () => {
    const { getAllByTestId, queryByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    const newRootBtn = getAllByTestId("new-department-btn").find(
      (b) => b.getAttribute("data-fn") === "M02.F01.I03",
    )!;
    expect(newRootBtn).toBeTruthy();
    fireEvent.click(newRootBtn);
    await waitFor(() => expect(queryByTestId("new-department-dialog")).toBeTruthy());
  });

  fnTest(["M02.F01.I04"], "I04 新增子部门按钮挂 data-fn + 打开 Dialog", async () => {
    const { getAllByTestId, queryByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    const newChildBtn = getAllByTestId("new-department-btn").find(
  ...
  fnTest(["M02.F01.I05"], "I05 编辑按钮打开 EditDepartmentDialog", async () => {
    const { getAllByTestId, queryByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
  ...
  fnTest(["M02.F01.I05"], "I05 提交 EditDepartmentDialog 调 PUT /api/departments/[id]", async () => {
    ...
    const { getAllByTestId, getByTestId, queryByTestId } = render(
      <DepartmentsClient initialTree={initialTree} />,
    );
    fireEvent.click(getAllByTestId("edit-department-btn")[0]!);
    await waitFor(() => expect(queryByTestId("edit-department-dialog")).toBeTruthy());
  ...
  fnTest(["M02.F01.I08"], "I08 部门表单弹窗（NewDepartmentDialog + EditDepartmentDialog）独立组件存在", () => {
    // M02.F01.I08 把表单弹窗抽成 NewDepartmentDialog / EditDepartmentDialog 独立组件：
    //   - NewDepartmentDialog 由顶部「新增根/子」按钮触发
    //   - EditDepartmentDialog 由行内「编辑」按钮触发
    // 这里断言两个 testid 都稳定存在（独立组件被 mount 后才能出现）
    const { getAllByTestId, queryByTestId } = render(<DepartmentsClient initialTree={initialTree} />);
    // 先不开 dialog，dialog 不在 DOM
    expect(queryByTestId("new-department-dialog")).toBeNull();
    expect(queryByTestId("edit-department-dialog")).toBeNull();
    // 通过点击触发，证明组件被独立创建 + 渲染
    expect(getAllByTestId("new-department-btn").length).toBeGreaterThan(0);
    expect(getAllByTestId("edit-department-btn").length).toBeGreaterThan(0);
  });
});