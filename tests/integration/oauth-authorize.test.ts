...
describe("M04.F03.I01 /api/v1/oauth/authorize", () => {
  ...
  it("M04.F03.I01 returns 200 { code, state } for valid request", async () => {
    dbMock.select
      .mockReturnValueOnce({
        from: () => ({
          where: () => ({
  ...
  it("M04.F03.I01 returns 400 INVALID_REQUEST when fields missing", async () => {
  ...
  it("M04.F03.I01 returns 400 UNSUPPORTED_RESPONSE_TYPE when not 'code'", async () => {
  ...
  it("M04.F03.I01 returns 400 INVALID_CLIENT when clientId unknown", async () => {
  ...
  it("M04.F03.I01 returns 400 INVALID_REDIRECT_URI when redirectUri not in whitelist", async () => {
    dbMock.select.mockReturnValueOnce({
      from: () => ({
        where: () => ({
  ...
  it("M04.F03.I01 returns 400 NO_USER when tenant has no user", async () => {
    dbMock.select
      .mockReturnValueOnce({
        from: () => ({
          where: () => ({
